<?php include("shared/common.php");?>
<!DOCTYPE html>
<html lang="en">
<head>
	<title>Safety</title>
	<?php head();?>
  <link rel="stylesheet" type="text/css" href="css/SKStyle.css">
</head>
<body>

  <!-- Navigation Bar
  –––––––––––––––––––––––––––––––––––––––––––––––––– -->
  <?php navbar();?>
<!-- Page Content -->
	<div class="container">
		<div class="panel" id="home">
			<div class="twelve columns">
				<div class="panelImage">
					<img src="images/watersafety.jpg" alt="jumbotron image">
				</div>
					<h3>Water Safety</h3>
					<p>Scroll down to find pages containing info on boating, cpr, and many other water safety.</p>
					<p>As students progress through the level they will be able to swim increased distances up to half a kilometer. Every level will educate students on new safety items like boating safety, CPR, ice safety and many more. Upon completion, each student will know three resting strokes and two power strokes.</p>
					<p>to add: scroll down sign</p>
			</div>
		</div>
		<div class="panel" id="one">
			<div>
				<div class="twelve columns">
					<h3>Hypothermia</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Swimming will learn the basic of kicking and swim up to 5 meters</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>swimmers will learn entries</p>
				</div>
			</div>
		</div>
		<div class="panel" id="two">
			<div>
				<div class="twelve columns">
					<h3>Contacting EMS - 911</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
		<div class="panel" id="three">
			<div>
				<div class="twelve columns">
					<h3>How to be a Safe Boater</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
		<div class="panel" id="four">
			<div>
				<div class="twelve columns">
					<h3>Staying Warm - HELP / Huddle</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
		<div class="panel" id="five">
			<div>
				<div class="twelve columns">
					<h3>When and where to go on ice</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
		<div class="panel" id="six">
			<div>
				<div class="twelve columns">
					<h3>Choking</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
		<div class="panel" id="seven">
			<div>
				<div class="twelve columns">
					<h3>Rescue Breathing</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
		<div class="panel" id="eight">
			<div>
				<div class="twelve columns">
					<h3>Ice Rescues</h3>
					<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				</div>
			</div>
			<div>
				<div class="six columns">
					<h5>Prevention</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
				<div class="six columns">
					<h5>Treatment</h5>
					<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
	</div>

  <?php footer();?>
</body>
</html>
