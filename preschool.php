<?php include("shared/common.php");?>
<!DOCTYPE html>
<html lang="en">
<head>
	<title>Preschool</title>
	<?php head();?>
  <link rel="stylesheet" type="text/css" href="css/PreStyle.css">
</head>
<body>

  <!-- Navigation Bar
  –––––––––––––––––––––––––––––––––––––––––––––––––– -->
  <?php navbar();?>
<!-- Page Content -->
	<div class="container">
		<div class="panel" id="home">
			<div class="twelve columns">
				<div class="panelImage">
					<img src="images/preschool.jpg" alt="jumbotron image">
				</div>
					<h3>Preschool</h3>
					<p>Preschool levels meant for childrean from 6 months old up to 6 years old. They will learn fundamental skills and self water safety while playing games and building confidence around </p>
			</div>
		</div>
		<div class="panel" id="one">
			<h3>Starfish & Duck</h3>
				<p>Starfish and Duck are Parent and Tot levels. Parents will be attending the class along with their babies and toddlers. Parents wil be educated on water safety and how to handle and assist their babies in and around the water. Babies and toddlers will have their first experiences around the water.</p>
				<ul>
					<li>Class Size: Up to 10 parents and toddlers.</li>
					<li>Age group: 4 months to 24months.</li>
				</ul>
		<div class="subItems">
			<div class="four columns">
				<h5>Safety</h5>
				<p>Swimming will learn the basic of kicking and swim up to 1 meters</p>
				<p>for the first time students will learn to sing songs and play gays to develop their comfortability in the water.</p>
			</div>
			<div class="eight columns">
				<h5>Overview</h5>
				<p><strong>Entries</strong> - parents will learn how to safety enter and exit the water with their child. From a graduate walk in, stairs or by the ledge of the pool.  All types of entries. Students will also experience jumps too.</p>
				<p><strong>Blowing Bubbles</strong> - toddlers will learn to exhale around and in the water. Learning from blowing bubbles on their finger first to blowing bubbles underwater.</p>
				<div class="criteria">Detailed Criteria
				<a class="criteriaButton" href="./content/contentPS2.php">Duck</a>
				<a class="criteriaButton" href="./content/contentPS1.php">Starfish</a>
				</div>
			</div>
		</div>
	</div>
	<div class="panel" id="two">
	
			<div class="twelve columns">
				<h3>Sea Turtle & Sea Otter</h3>
				<p>Students can start sea turtle at 2.5 age and can advance to preschooler levels before 3. While sea turtle is still parent assisted, its goal is to graduatly move toddlers away from parent dependency in swimming classes. Once a child is comfortable in lesson setting without their parents they will move onto Sea Otter. Theses 2 levels students will be introduced to floats and glides.</p>
				<ul>
					<li>Class Size: 1 - 3 </li>
					<li>Age group: 24 months to 5 years old.</li>
				</ul>
			</div>
		
		<div class="subItems">
			<div class="four columns">
				<h5>Safety</h5>
				<p>Stop Look Ask</p>
				<p>PFD and Me</p>
			</div>
			<div class="eight columns">
				<h5>Overview</h5>
				<p><strong>Submersion</strong> - asdf</p>
				<p><strong>Floats</strong> - asdf</p>
				<p><strong>Glides</strong> - asdf</p>
				<div class="criteria">Detailed Criteria
				<a class="criteriaButton" href="./content/contentPS2.php">Sea Otter</a>
				<a class="criteriaButton" href="./content/contentPS1.php">Sea Turtle</a>
				</div>
			</div>
		</div>
	</div>
	<div class="panel" id="three">
	
			<div class="twelve columns">
				<h3>Salamander & Sunfish</h3>
				<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				<ul>
					<li>Class Size: Up to 3 Preschoolers.</li>
					<li>Age group: 3 to 5 year olds.</li>
				</ul>
			</div>
		
		<div class="subItems">
			<div class="four columns">
				<h5>Safety</h5>
				<p>Stop Look Ask!</p>
				<p>Weight Transfer</p>
				<p>PFD and Me</p>
			</div>
			<div class="eight columns">
				<h5>Swimming Skills</h5>
				<p><strong>Floats and Glides</strong> - asdf</p>
				<p><strong>Rollovers</strong> - asdf</p>
				<p><strong>Front Swim</strong> - asdf</p>
				<div class="criteria">Detailed Criteria
				<a class="criteriaButton" href="./content/contentPS2.php">Sunfish</a>
				<a class="criteriaButton" href="./content/contentPS1.php">Salamander</a>
				</div>
			</div>
		</div>
	</div>
	<div class="panel" id="four">
	
			<div class="twelve columns">
				<h3>Crocodile & Whale</h3>
				<p>Welcome to the first level of the Red Cross Swim Kids program. In this level, students will learn be introduced to floats, glides and rhythmic breathing.</p>
				<ul>
					<li>Class Size: Up to 4 Preschoolers.</li>
					<li>Age group: 3 to 5 year olds.</li>
				</ul>
			</div>
		
		<div class="subItems">
			
			<div class="four columns">
				<h5>Safety</h5>
				<p>When and Where to Swim</p>
				<p>Sitting Dive</p>
				<p>Stop! Call for Help!</p>
			</div>
			<div class="eight columns">
				<h5>Swimming Skills</h5>
				<p><strong>Front Glide / Side Glide</strong> - asdf</p>
				<p><strong>Front Swim</strong> - asdf</p>
				<p><strong>Back Swim</strong> - asdf</p>
				<div class="criteria">Detailed Criteria
				<a class="criteriaButton" href="./content/contentPS2.php">Whale</a>
				<a class="criteriaButton" href="./content/contentPS1.php">Crocodile</a>
				</div>
			</div>
		</div>
	</div>
	<div class="panel" id="nowWhat">
	
			<div class="twelve columns">
				<h3>After Preschool</h3>
				<p>Students will move into the Swim Kids Program after they turn 6 years old.</p>
			</div>
		
		<div class="subItems">
			<div class="four columns">
				<h5>6 and Older</h5>
				<p>After your child turns 6 years old, they will be moving into the Swim Kids Program. Here's a guide on leel equivalencies of preschool to Swim Kids.</p>
				<ul>
					<li>Starfish to Sea Otter -> Swim Kids 1</li>
					<li>Salamander / Sunfish -> Swim Kids 2</li>
					<li>Crocodile / Whale -> Swim Kids 3</li>
					<li>Preschool 4 / 5  -> Swim Kids 4</li>
				</ul>
				<a class="button" href="#">Swim Kids</a>
			</div>
			<div class="four columns">
				<h5>Swim Clubs</h5>
				<p>If you child has completed Whale of higher, they have the option to continue their learning through swim clubs. Swimming club programs focus on power and speed. The strokes learned will be a more competitive style compared to the swim kids style.</p>
				Pros
				<ul>
					<li>More Challenging, Better Fitness, Better technique</li>
					<li>Swim clubs are team oriented, develop friendships</li>
					<li>More structured and strict, builds character</li>
				</ul>
				Cons
				<ul>
					<li>No water safety</li>
					<li>more expensive and time consuming</li>
				</ul>
			</div>
			<div class="four columns">
				<h5>SwimKids</h5>
				<p>Lorem ipsum dolor sit amet, ut qui atqui interpretaris, pri cu facer graeco contentiones. No nonumy intellegam eam, sed ut suas inani antiopam, quando nullam legimus sed ad. Mei splendide interpretaris id, vel cu dicit molestie. Ut bonorum saperet consetetur sea. Ex mea alii labore intellegat, molestie delicata expetenda in nec, ex harum soluta est.</p>
				</div>
			</div>
		</div>
	</div>

  <?php footer();?>
</body>
</html>